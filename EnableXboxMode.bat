...
:: ============================
set "TMP_REG=%DOWNLOAD_DIR%\regq_%RANDOM%.tmp"
echo Checking %REG_KEY%\%REG_VAL% ...

:: Query to a file (avoids pipes/paren issues)
reg query "%REG_KEY%" /v %REG_VAL% > "%TMP_REG%" 2>&1
if errorlevel 1 (
  echo %REG_VAL% not found. Creating with %REG_DECIMAL% ...
  reg add "%REG_KEY%" /v %REG_VAL% /t REG_DWORD /d %REG_DECIMAL% /f >nul 2>&1 || goto :fail
) else (
  set "CURRENT_HEX="
  for /f "usebackq tokens=1,2,3,*" %%A in ("%TMP_REG%") do (
    if /i "%%A"=="%REG_VAL%" set "CURRENT_HEX=%%C"
  )
  if not defined CURRENT_HEX (
    echo Could not parse existing value. Forcing to %REG_DECIMAL% ...
    reg add "%REG_KEY%" /v %REG_VAL% /t REG_DWORD /d %REG_DECIMAL% /f >nul 2>&1 || goto :fail
  ) else (
    :: Convert e.g. 0x0000002e -> decimal and compare
...
if exist "%TMP_REG%" del /f /q "%TMP_REG%" >nul 2>&1
